...
if choice == "Default" then
    cons.print("[+] Loading default map. [You need to be in TSB to use this one!]")
    local _, isintsb = pcall(function()
        return game:GetService("Players").LocalPlayer.Character.Communicate
    end)
    if not _ and isintsb then
        cons.warn(_format("[-] Could not find Communicate event for building, this can happen because you are not in the official TSB game.\n-- DEBUG --\nsuccess: %s\nerror: %s", tostring(_), tostring(isintsb)))
        return
    end
    ...
    function create(part, isWall)
        ...
        if isWall then
            task.delay(0.2, function()
                cons.print("[loadPlaceDebug]: Setting destructible property to True to the part due to it being a Wall.")
                task.wait()
    ...
    cons.print("[+] Hiding map so the built one is visible")
    task.spawn(tsb_hidethemapplease)

elseif choice == "Blox Fruits" then
    cons.warn("[-] Blox Fruits' map is currently a W.I.P, this one will have up to the second sea due to third sea being huge [As of now], the map is not built and the script is not done.")
    return;
elseif choice == "JJS" then
    cons.print("[=] There will be alot of debugging cus its a big map. (Soon this will be same debug level for Blox Fruits Map)")
    cons.print("[+] Warning messages all uppercase inside of messages like '-- BEGIN DEBUG --' or '-- END DEBUG --' mean the script failed due to executor issue and is now debugging.")
    cons.print("[?] Alright no more yapping the next message below me is the script getting JJS map metadata")
    cons.print("[+] Rest of debugging will be logged into roblox real console")
